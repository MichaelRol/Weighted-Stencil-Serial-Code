...
void stencil(const int nx, const int ny, float * restrict  image, float * restrict tmp_image) {
  ...
  for(int j = 1; j < ny - 1; ++j){
    tmp_image[ny * j] = image[ny * j] * 0.6f + image[ny * (j - 1)] * 0.1f + image[ny * (j + 1)] * 0.1f + image[ny * j + 1] * 0.1f;
  }
  for(int j = 1; j < ny - 1; ++j){
    tmp_image[ny * j + ny - 1] = image[ny * j + ny - 1] * 0.6f + image[ny * (j - 1) + ny - 1] * 0.1f + image[ny * (j + 1) + ny - 1] * 0.1f + image[ny * j + ny - 2] * 0.1f;
  }
